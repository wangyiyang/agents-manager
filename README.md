...
- 🎯 Entry points to execute your program using `python -m <agents_manager>` or `$ agents_manager` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/wangyiyang/agents-manager/branch/main/graph/badge.svg?token=agents-manager_token_here)](https://codecov.io/gh/wangyiyang/agents-manager)
[![CI](https://github.com/wangyiyang/agents-manager/actions/workflows/main.yml/badge.svg)](https://github.com/wangyiyang/agents-manager/actions/workflows/main.yml)

Awesome agents_manager created by wangyiyang

## Install

from source
```bash
git clone https://github.com/wangyiyang/agents-manager agents_manager
cd agents_manager
make install
```
...
```bash
pip install agents_manager
```

## Executing

```bash
$ agents_manager run --port 8080
```

or

```bash
python -m agents_manager run --port 8080
```

or

```bash
$ uvicorn agents_manager:app
```
...
```bash
❯ agents_manager --help
Usage: agents_manager [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ agents_manager create-user --help
Usage: agents_manager create-user [OPTIONS] USERNAME PASSWORD
...
```bash
agents_manager create-user admin admin --superuser
```
...
Run with `agents_manager run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `agents_manager create-user`.
...
-----------------------------------------------------
agents_manager/__init__.py              4      0   100%
agents_manager/app.py                  16      1    94%
agents_manager/cli.py                  21      0   100%
agents_manager/config.py                5      0   100%
agents_manager/db.py                   10      0   100%
agents_manager/models/__init__.py       0      0   100%
agents_manager/models/content.py       47      1    98%
agents_manager/routes/__init__.py      11      0   100%
agents_manager/routes/content.py       52     25    52%
agents_manager/routes/security.py      15      1    93%
agents_manager/routes/user.py          52     26    50%
agents_manager/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in agents_manager/default.toml.
